...
- **D-233 (GEO: два провайдера и web search)** — Вопросы видимости AI SEO / GEO
  ...
  **Registry v2 — константы кода, а не настройки пользователя.**
  `DEFAULT_OPENAI_MODEL = 'gpt-5.6-terra'`: тот же ценовой класс, что
  `claude-sonnet-5` ($2/$12 против $2/$10), поэтому ответы сравнимы, а AI-расход
  скана примерно удваивается, а не взрывается. `OPENAI_MODEL` переопределяет
