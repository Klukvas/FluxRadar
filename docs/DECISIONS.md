...
- **D-229. Paddle убран из кода; в экономике — комиссия FastSpring.** Paddle
  ...
  `test-utils/purchase-scan.ts` пишет строку checkout-сессии и прогоняет
  подписанный `order.completed` через настоящий обработчик. Проверку
  ...
  переменную можно удалить из `PRODUCTION_ENV_FILE`. Сами колонки `paddle*`,
  ...
  $51.97 вместо $24.25 и $53.50). Исторические документы (разборы задач, старые
  решения, план) остаются как есть: это история, а не описание текущего кода.
