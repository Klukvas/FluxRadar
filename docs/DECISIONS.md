...
- **D-231. Contract-фаза: колонки ушедшего провайдера удалены.** Миграция
  ...
  `BILLING-007` удалены вместе с триггерами. Выкатывать её можно, только когда
  оба сохранённых кандидата на откат — релиз D-229 или новее: проба отката
  проверяет предыдущий, а деплой хранит два, и ручной откат на релиз, который
  ещё выбирает эти колонки, упал бы. Данные не теряются: каждое значение было
  копией колонки, которая остаётся. После неё Paddle не остаётся ни в коде, ни
  в схеме — только в истории миграций, которую не переписывают.
