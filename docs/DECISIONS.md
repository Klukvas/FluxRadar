...
- **D-201 (T-12 follow-up)** — Paddle `transaction.disputed` переводит purchase
  в `Disputed`, приостанавливает entitlement и блокирует worker execution (с D-216
  — и чтение отчёта); переход не откатывает уже `Refunded` purchase. Refund сохраняет transaction/event/signature,
...
- **D-204 (production hardening)** — История сканов страницуется в PostgreSQL, а не в
  JavaScript. `GET /scans`, `GET /profiles/:id/scans` и `GET /profiles` выполняют
  `skip/take` + `count` с сортировкой `(createdAt, id)`; history gate (Complete
  открывает историю, Basic показывает только текущий результат) стал двумя
  existence-пробами вместо решения по полностью загруженному массиву. Причина:
  прежний `findMany` без `take` читал всю историю аккаунта на каждый запрос —
  стоимость листинга росла вместе со сроком жизни платящего клиента. Контракт
  ответа сохранён: те же `limit`/`offset`, тот же `meta {total, page, limit}`;
  добавлены необязательные `page`/`pageSize` как альтернативное написание и
  `meta.hasNext`. `id` в ORDER BY обязателен: без него два скана с одинаковым
  `createdAt` могут поменяться местами между страницами и один из них исчезнет.
- **D-205 (production hardening)** — Добавлены индексы `Scan(accountId, createdAt, id)`,
  `Scan(accountId, siteProfileId, createdAt, id)`, `Scan(accountId, plan)`,
  `Scan(accountId, status)`, `Scan(siteProfileId)`, `Purchase(siteProfileId)`,
  `CheckoutSession(siteProfileId, status)` и `Job(status, createdAt, id)`.
  Миграция строго additive (expand-phase, `CREATE INDEX IF NOT EXISTS`): ни одна
  колонка не переименована и не удалена, данные не переписываются, откат на
  предыдущий релиз безопасен. Причина: все перечисленные запросы — листинг
  истории, `/scans/active`, claim очереди каждые 30 секунд и профильные guard-ы —
  были sequential scan по таблицам, которые растут вместе с историей продукта.
- **D-206 (production hardening)** — Каждое ограничиваемое действие считается по двум
  независимым ключам: аккаунт (или email) и клиентский IP, вместо одного составного
  `account:ip`. Причина: составной ключ обходится сменой IP — аккаунтный лимит при
  нём не ограничивает ничего. IP-лимиты выше аккаунтных, чтобы офисный NAT не
  блокировал команду. Отказ отдаёт `429` с заголовком `Retry-After`, посчитанным по
  самой старой попытке в окне; тело envelope не изменилось. Rate limit регистрации
  выполняется ДО bcrypt (cost 12): проверка после хеширования позволяла флуду
  тратить CPU процесса, ничего при этом не создавая. Ограничены также
  `/scans/:id/cancel` и `/scans/:id/export`.
- **D-207 (production hardening)** — `trust proxy` задаётся явным `TRUST_PROXY`
  (число hops или `false`), а не выводится из `NODE_ENV`; blanket `true` Express-а
  не принимается. Причина: `req.ip` определяет, что именно считают IP-лимиты.
  Без доверия прокси все запросы приходят с одного адреса Caddy — один клиент
  закрывает вход всем; с доверием без прокси клиент сам называет свой адрес и
  получает чистый счётчик на каждый запрос. Значение по умолчанию сохраняет
  прежнее поведение (1 hop в production).
- **D-208 (production hardening)** — MockPaddle surface (`/webhooks/paddle` и
  симулированная покупка в `/billing/dev-checkout`) включается только явным
  `FLUXRADAR_ENABLE_MOCK_CHECKOUT=true` и по умолчанию выключен; `validateRuntimeConfig`
  отказывается стартовать production с этим флагом. Причина: прежнее условие
  `NODE_ENV !== 'production'` читается как «выключено в production», но означает
  «включено везде, где переменная не равна строке production» — при отсутствующем
  или нестандартном `NODE_ENV` любой авторизованный аккаунт мог бесплатно создавать
  Complete-сканы. Production-путь бесплатного доступа остаётся ровно один —
  allowlist `FLUXRADAR_INTERNAL_FREE_EMAILS` (fail-closed при отсутствии,
  не зависит ни от этого флага, ни от `NODE_ENV`).
- **D-209 (production hardening)** — Domain ownership verification для платных сканов
  **не вводится**. Платный скан уже привязан server-side к аккаунту сессии, к
  профилю этого аккаунта и к provider entitlement: `CheckoutSession` пишется до
  вызова провайдера, браузер и payload провайдера не могут назвать чужой профиль,
  а `createPaidScan` повторно проверяет `profile.accountId`. Подтверждение того,
  что покупатель владеет доменом (DNS TXT, файл на хосте, Search Console), — это
  отдельный продукт со своим UX, отдельным состоянием и поддержкой; вводить его
  как побочный эффект backend-фикса нельзя, а публичный аудит чужого сайта сам по
  себе соответствует модели продукта (crawler соблюдает robots.txt, safe-fetch
  закрывает SSRF, `FreeCheckClaim` глобально ограничивает бесплатный доступ по
  origin). Добавлена только одна реально необходимая проверка: смена `domain`
  профиля запрещена, пока по нему есть checkout, который ещё может быть оплачен
  (`PROFILE_HAS_OPEN_CHECKOUT`, то же окно и тот же код, что и у guard-а удаления).
  Причина: домен скана читается из профиля в момент прихода подписанного webhook —
  то есть уже после авторизации платежа, — и это единственное место, где предмет
  оплаченного скана оставался изменяемым покупателем. Закрывается запретом
  изменения, а не отказом в оплате: возвращать деньги за переименованный профиль
  хуже, чем попросить дождаться завершения checkout.
- **D-210 (production hardening)** — Retention scan purge удаляет `ExportArtifact`
  вместе со сканом и чистит объект в приватном хранилище **после** коммита
  транзакции. Причина: FK `ExportArtifact_scanId_fkey` — `ON DELETE RESTRICT`, и
  `deleteScanResult` его не трогал, поэтому первый же истёкший скан, который
  когда-либо экспортировали, ронял `tx.scan.delete` с foreign key violation.
  Sweep последовательный, а `sweepRetention` глотает ошибку в лог, поэтому дальше
  не выполнялись ни purge webhook-payload-ов, ни закрытие брошенных checkout —
  retention переставала работать целиком и молча. Вторая половина той же дыры:
  объект отчёта оставался в бакете навсегда, то есть данные переживали окно
  хранения, которое им обещано. Удаление объекта — best effort и считается
  (`orphanedArtifactCount` в логе sweep): отказ бакета не должен останавливать
  удаление того, что удалить можно, иначе данных остаётся больше, а не меньше.
- **D-211 (production hardening)** — Логин ограничивается тремя независимыми
  ключами: пара (email, IP), сам email и сам IP. Причина: составной ключ
  `email|ip` — ровно та форма, которую заголовок `auth/rate-limit.ts` уже называл
  ошибочной, и она не ограничивает ни password spraying (один пароль по тысяче
  аккаунтов с одного адреса — ни одна пара не повторяется), ни распределённый
  перебор одного аккаунта (каждая попытка с нового адреса открывает свежую
  корзину). Успешный вход сбрасывает только счётчики аккаунта: правильный пароль
  доказывает владение этим аккаунтом и ничего не говорит о переборе остальных с
  того же адреса. Адресный потолок заметно выше аккаунтного — за одним NAT сидит
  целый офис.
- **D-212 (production hardening)** — `INTEGRATION_ENCRYPTION_KEY` в production
  обязателен, fallback-а нет, и он не должен совпадать с `SESSION_SECRET`.
  Причина: копирование `SESSION_SECRET` в переменную с нужным именем проходит
  проверку присутствия и возвращает ровно ту связанность, ради устранения которой
  fallback и запрещён — в день ротации `SESSION_SECRET` все сохранённые
  Google/Bing токены становятся нерасшифровываемыми, и никакой ошибки нигде нет:
  AES-GCM просто перестаёт аутентифицировать. Правило живёт в одном месте
  (`integrations/encryption-key.ts`), применяется дважды — `normalize-env-file.cjs`
  роняет **деплой**, `validateRuntimeConfig` роняет **старт**, — и сообщает только
  имена переменных. Ключ короче 32 символов — предупреждение деплоя, а не отказ:
  деплой нужен, чтобы такой ключ можно было заменить.
- **D-213 (production hardening)** — CSP перестала быть непроверяемой строкой.
  `DEPLOY-008` сверяет `deploy/Caddyfile` с константами кода (`SBL_ORIGIN`,
  `STOREFRONT_DOMAIN_SUFFIX`), запрещает `'unsafe-inline'`/`'unsafe-eval'`/wildcard
  в `script-src` и требует, чтобы `deploy/nginx.conf` повторял ту же политику
  байт в байт; `public-smoke.sh` проверяет её наличие снаружи сервера;
  `inline-script-policy.test.ts` следит, чтобы ни один отдаваемый документ не
  содержал inline-скрипта, `on…=` обработчика или off-origin ресурса. Причина:
  `docs/DEPLOYMENT.md` прямо писал, что поломка политики «видна покупателю, но не
  серверу», а dev/preview отдают те же страницы вообще без политики — то есть
  inline-скрипт в блоге работал везде, кроме production.
- **D-214 (production hardening)** — Проверка advisory зависимостей — отдельная
  job в `ci.yml`, а **не** гейт деплоя: advisory публикуется против уже
  смёрдженного и уже работающего кода, поэтому блокировка деплоя блокировала бы
  собственный хотфикс. Runtime-зависимости (`--prod`) блокируют merge,
  build/test — только сообщают. Сканирование образов и мониторинг не добавлены и
  явно перечислены в *What is not automated*, чтобы их отсутствие не читалось как
  наличие.
- **D-215 (production hardening)** — Конфигурация Resend читается одним
  `readResendConfig`, из которого строится и mailer, и строка диагностики, так что
  «письма выключены» и «письма показаны выключенными» больше не могут разойтись.
  Он же отвергает отправителя, которого Resend не примет: невалидный
  `RESEND_FROM_EMAIL` неотличим от рабочего до момента, когда клиент скажет, что
  письмо не пришло. Boot при этом не падает — деплой без почты всё ещё продаёт и
  выполняет сканы. Верификация домена и публикация DKIM/SPF остаются ручными
  шагами на стороне провайдера и описаны в `docs/DEPLOYMENT.md`.
- **D-216 (production hardening)** — Suspended entitlement теперь забирает и сам
  отчёт, а не только будущую работу. Refund и chargeback ставят `suspended`, и
  рядом с этим кодом написано зачем: «покупатель, которому вернули деньги, не
  должен сохранить отчёт, за который платил». Спрашивали об этом только worker и
  module retry, поэтому все чтения — `GET /scans/:id`, `/dashboard`, `/issues`,
  `/issues/:id`, `/evidence`, `PATCH /issues/:id` и `/export` — отдавали платный
  отчёт как раньше. Правило вынесено в один `billing/report-access.ts`
  (`paidAccessDenial`), которым пользуются и HTTP-слой, и worker, и возвращает
  403 `ENTITLEMENT_SUSPENDED`, а не 404: скан принадлежит аккаунту, и делать вид,
  что его не было, — врать в интерфейсе. Списки (`/scans`, `/scans/active`,
  `/profiles/:id/scans`) оставляют строку скана, но без report payload: модули с
  их score — те же платные данные, что и detail. Три решения зафиксированы явно,
  чтобы не выглядеть упущением: **expiry ≠ suspension** (30 дней ограничивают то,
  что ещё можно купить, — «after expiry no new scans/retries are queued, a Running
  scan may finish», — а не срок чтения уже доставленного отчёта, поэтому истёкший
  entitlement читается и только `retry`/`process` отдают `ENTITLEMENT_INACTIVE`);
...
- **D-218 (UX/Conversion score)** — Уточнение D-217. §15 обещает UX/Conversion
  собственную оценку 0–100, которая не входит в общий score; реализация сделала
  из «не входит в общий» «нет вообще», и карточка показывала «No score» рядом со
  списком сработавших проверок. Теперь все шесть UX-правил `scored` (static-001/002
  Medium, static-003 Low, AI-правила — базовая Medium, но issue хранит оценку,
  которую дал AI-разбор): штраф правила = weight(max severity) × доля страниц,
  на которые правило смотрело (entry page / страницы с формой / все
  проанализированные — та же `uxRuleCounts`, что рисует список проверок), и все
  строки правила несут общие агрегаты и penalty (D-016), так что экспорт
  пересчитывает их как у любого модуля. Partial без AI-разбора оценивается по
  трём статическим проверкам. Общий score не меняется: `SIDE_SCORE_MODULES`
  по-прежнему пропускает UX. Исключение экспорта из D-217 оставлено только для
  отчётов, созданных до этого решения. Цена решения — шум AI: оценка одного и
  того же сайта может немного отличаться между сканами; карточка прямо пишет
  «AI-assisted review» и «отдельная оценка».
- **D-219 (Analytics checks)** — Подключение Google раньше только добавляло в
  отчёт цифры Search Console и GA4: ни одной проверки, `score: null`, coverage
  «2 из 2» означал «оба источника ответили». Теперь модуль прогоняет восемь
  правил: `ANALYTICS-SC-001` падение органики ≥30% к предыдущим 28 дням (клики
  от 10 в прошлом периоде, иначе показы от 100; High, site), `SC-002` страница с
  ≥50 показами на средней позиции ≤10 без кликов (Medium, page), `SC-003`
  запросы с ≥10 показами на позициях 8–20 (informational, список), `SC-004`
  индексируемая страница из обхода в области ресурса Search Console без показов
  (Low, page; не оценивается, если список страниц упёрся в лимит 5000 строк),
  `SC-005` у сайта ≥100 показов и ни одного клика (Medium, site — обычно показы
  идут с позиций за первой страницей, это проблема позиций, а не сниппета одной
  страницы), `GA-001` есть сессии и ни одного key event (Medium, site;
  `keyEvents=null` — метрика не поддерживается, не оценивается), `GA-002`
  страница без тега Google (gtag.js/GTM) в HTML, когда другие страницы его
  несут (Medium, page; если тега нет ни на одной — тег грузится бандлом, не
  оценивается), `LINK-001` десять страниц с наибольшими показами, число находок
  отчёта на них, самая серьёзная и id сработавших правил (informational; по id
  ищет Issue Center). Пороги поставлены низко намеренно: код-ревью показало, что
  первая версия (100 сессий, 200 показов, позиции >10) глушила ровно те находки,
  которые владелец назвал на своём маленьком сайте. Оценка — та же формула §15,
  в общий score не входит. Coverage = правила, чей источник вернул данные, из
  восьми. Новых вызовов GA API нет (важно для верификации `analytics.readonly`):
  запрос key events лишь получил `keepEmptyRows`, потому что GA4 не возвращает
  строку, где все метрики нулевые, и «ноль key events» приходил как «неизвестно»;
  успешный пустой ответ тоже читается как 0. Search Console получил один запрос
  итогов за предыдущий период, а списки запросов и страниц читаются до
  1000/5000 строк — в снимок отчёта по-прежнему попадает только топ-10, полные
  списки не хранятся. Данные Google в AI не уходят. Analytics пишется после
  `resolveScanOutcome`, когда обхода уже нет, поэтому `runScanAttempt`
  возвращает факты по страницам (`analyticsPageFacts`), а находки получают
  `observed_at = completed_at` скана — иначе экспорт (EXPORT-001/2) отверг бы их
  как записанные после завершения. Данные Google переехали из отдельного блока
  под карточками внутрь раскрывающейся панели карточки Analytics.

- **D-220. Обход ходит через egress-прокси, а SSRF-гард остаётся у нас.** Сайты
  за Cloudflare, которые режут хостинг-сеть прода (проверено: ukrdentclub.ua
  отдаёт 403 только узлам Hetzner и 200 полусотне других точек, включая
  киевский VPS с тем же `FluxRadarBot/0.1`), превращают отчёт в набор находок
  про нас самих. Поэтому `CRAWL_EGRESS_PROXY_URL` уводит запросы обхода через
  HTTP CONNECT-прокси в сети проверяемого рынка. Выбран CONNECT, а не новая
  зависимость или перенос воркера: тоннель оставляет TLS сквозным, и
  сертификат проверяется по hostname на нашей стороне. Резолв и проверка
  адресов (D-125) никуда не переезжают — прокси получает только IP-литерал,
  уже одобренный гардом, и перебираются все адреса хоста по очереди, как это
  делает прямой путь. Порт в настройке обязателен: молчаливый переход на 80
  из-за опечатки — это развёртывание, обходящее сайты не из той сети, и в
  отчётах такой сбой не виден. Значение с паролем не попадает ни в логи, ни в
  текст ошибок, даже длиной; нечитаемое значение валит старт по имени
  переменной, а не откатывается на заблокированную сеть. Отсутствие переменной
  — поддержанное состояние (прямой обход, локальная разработка), а fixture-сайт
  на loopback всегда обходится напрямую: снаружи до него не достучаться.


- **D-221. Покрытие меряется в адресах сайта, а не в проверках.** Единственное
  «покрытие» в отчёте было модульным — `completed_applicable_checks /
  applicable_checks`, — и оно не видит обхода: скан прочитал 15 страниц из 334,
  каждая проверка, у которой была страница, закрылась, и все модули показали
  100%. Дефолт `maxPages` был 15 на любом тарифе, включая Complete за $120 с
  лимитом 50 000 URL, поэтому под это попадал каждый сайт крупнее пятнадцати
  адресов. Дефолт убран совсем: пустое значение означает «весь тариф»
  (`buildCrawlScope`), а не число, придуманное этим файлом. Обход пишет
  `Scan.crawlSummaryJson` — сколько адресов нашли и сколько прочитали, — и отчёт
  показывает это отдельной панелью, называя, чей лимит остановил чтение:
  собственная настройка владельца или потолок тарифа. Адреса, запрещённые
  robots.txt сайта, в знаменатель не входят и считаются отдельно: не читать
  запрещённое — это соблюдение правил, а не пробел в аудите.

- **D-222. Сайт прочитан только тогда, когда прочитана страница.** Признаком
  достижимости было `pages.some((page) => page.fetchError === undefined)`, а
  страница-заглушка WAF ему удовлетворяет: 403 приходит по совершенно здоровому
  соединению. Поэтому сайт, полностью нас заблокировавший, описывался как сайт
  без robots.txt, правила, которым нужна разметка, уходили в NotApplicable (и
  покидали знаменатель покрытия), а итог — 96.95 балла при нуле прочитанных
  страниц. Google PSI меряет заблокированный сайт как ни в чём не бывало, так
  что один «полезный» модуль делал скан Partial, то есть без возврата.
  Достижимость теперь — `isSuccessfulHtmlPage`: 2xx с документом. Если её нет,
  не запускается ни один модуль: ни правил по заглушке, ни AI-квоты на скан,
  который сейчас вернут, ни PSI. Все модули Unavailable с причиной, называющей,
  что сделал сайт (`SiteDeniedAccess`, `SiteBlockedByRobots`, `SiteUnreachable`,
  `SiteReturnedNoReadablePage`), скан Failed с полным возвратом.
  `EXTERNAL_NO_USABLE_OUTPUT` принимает любую причину этой ветки: возврат один и
  тот же, а слово — единственное, с чем владелец может что-то сделать.

- **D-223. Доступность проверяется до оплаты, и сервер проверяет её сам.**
  Клиент мог купить аудит сайта, который нас не пускает, дождаться скана и
  получить возврат вместо отчёта. `POST /profiles/:id/reachability` спрашивает
  сайт тем же fetcher-ом, тем же UA и через тот же прокси, что и обход, и тем же
  `assessSiteReach` читает ответ — проба другим маршрутом отвечала бы на другой
  вопрос. Результат хранится (`SiteReachabilityProbe`, одна строка на профиль),
  потому что `createCheckoutSession` перечитывает его сам: браузеру можно
  сказать что угодно. Просроченная проба (15 минут) отклоняется наравне с
  отрицательной — сайт, доступный час назад, сейчас может стоять за челленджем.
  Строка помнит и сам проверенный origin, а не только профиль: домен профиля
  можно менять, пока нет открытого checkout (`assertDomainChangeAllowed`), и без
  этой привязки проба лёгкого сайта, переезд профиля на блокирующий и оплата в
  те же 15 минут покупали скан непроверенного сайта — ровно то, что гейт и
  должен отклонять. Сравнение с текущим доменом, а не удаление строки при
  PATCH: так закрыт любой будущий путь смены домена, а не только известные.
  Колонка nullable и без backfill — строка, записанная до неё, действительно не
  знает, что проверяла, и считается непригодной.
  Лимит вызовов строже обычного: каждая проба — исходящий запрос на чужой
  сервер.

- **D-224. CONTENT-004 говорит только о том, что проверено.** Правило выдавало
  «Биті зображення або медіа» со средней критичностью и штрафом по
  доказательству «внутрішні медіафайли, не підтверджені обходом» — на краулере,
  который media не запрашивал вовсе; 21.09.2026 все три файла из такого
  доказательства руками отдали 200. Обход теперь спрашивает: один HEAD на
  каждый уникальный внутренний media-адрес в пределах бюджета
  (`CRAWL_LIMITS.maxMediaChecks`), через тот же транспорт и тот же per-host
  лимитер. Результаты живут в `CrawlResult.mediaChecks`, отдельно от `pages`:
  внутри `pages` они испортили бы покрытие из D-221. Ветка «внутренняя media без
  снимка» удалена — находка требует подтверждённого плохого ответа, confidence
  всегда 1. Что не влезло в бюджет, попадает в метаданные модуля как
  непроверенное, без severity и без штрафа.

- **D-225. Прокси обхода проверяется, а не предполагается.** Весь платный обход
  уходит через один VPS, и за ним никто не следил: когда он падал, падал каждый
  запрос, каждая страница становилась ошибкой сети, и скан сообщал, что
  недоступен сайт клиента — наша авария, описанная как его проблема, ценой
  оплаченного скана. Проверка идёт при старте API и каждые 5 минут, и проверяет
  две вещи, а не одну: что прокси отвечает и что интернет видит именно его адрес
  — CONNECT, тихо провалившийся на сеть хоста, прошёл бы ping и вернул нас в тот
  самый блок Hetzner, ради которого прокси и заведён. Попытка скана проверяет то
  же перед первым запросом и при неисправности падает как platform-сбой:
  молчаливый прямой обход недопустим (D-220), а платформенный сбой — наша
  категория, не клиентская. Трафик считается помесячно
  (`CrawlEgressUsage`, предупреждение на 80% от 1 ТБ); это нижняя оценка по телам
  ответов, а не счёт — авторитет по-прежнему `vnstat` на VPS.

- **D-226. Центр проблем группирует по правилу, а не по «правило + критичность».**
  Правила UX-CONV-AI-* получают severity от модели, по каждой находке, поэтому
  одно правило законно даёт находки разной критичности — и оно показывалось
  двумя строками (средняя и низкая), открывавшими один и тот же список. Ключ
  строки теперь `ruleId + module`; строка носит худшую из критичностей, потому
  что именно эта срочность предъявляется владельцу. Разбивка по критичности над
  списком по-прежнему считается по самим находкам, иначе сворачивание строки
  перенесло бы низкие находки в колонку средних.

- **D-227. Упоминание считается только тогда, когда его не было в вопросе.**
  Awareness-вопрос звучал как «What is <brand>, what does its official website
  https://<hostname> offer…», а у автосозданного профиля бренд равен hostname
  (`siteProfileNameFor`) — то есть вопрос содержал обе строки, которые потом
  искали в ответе. Модель, повторяющая предмет вопроса, давала зелёные «бренд
  згадано» и «офіційний домен наведено» на каждом скане. Домен из вопроса убран:
  теперь назвать сайт — это то, что модель должна знать. Бренд из вопроса убрать
  нельзя, поэтому awareness-вопросы перестали быть applicable-целью для
  GEO-VIS-003, и узнаваемость бренда меряется на нейтральных discovery-вопросах
  — единственном месте, где она что-то значила. Сигналы стали перечислением
  (`named-in-question`, `brand-is-hostname`), а не булевыми, и обе «неизмеримые»
  формы обязаны читаться как «не вимірюється», а не как успех. Платная форма
  спрашивает offerings и industry заранее: без них `neutralContext` не собирает
  темы и discovery-вопросы не генерируются вовсе.

- **D-228. Страна, из которой идёт проверка, — выбор владельца и часть отчёта.**
  Обход уходил через одну точку, заданную `CRAWL_EGRESS_PROXY_URL`, и это нигде
  не было видно: сайт может по-разному отвечать посетителям из разных стран
  (язык, редиректы, баннеры согласия, блокировки), а отчёт молчал, откуда его
  читали. Точка выхода стала записью реестра
  (`apps/api/src/integrations/crawl-egress-locations.ts`: id, страна, город,
  метка на двух языках, переменная прокси, месячный лимит трафика). Прежняя
  переменная — это точка `ua` (Київ) и точка по умолчанию, поэтому текущие
  развёртывания обходят ровно оттуда же; новая страна — это
  `CRAWL_EGRESS_PROXY_URL_<CODE>` плюс запись в реестре, без правок в правилах,
  краулере и оркестраторе. Нечитаемое значение и переменная для
  незарегистрированной страны валят старт по имени (D-220): оператор хотел,
  чтобы страна появилась, а она молча не появилась бы. Выбор живёт в
  `scanScopeSchema.egressLocation`, в конфигурации профиля и в
  `executionConfigJson` скана, и проверяется сервером на каждом пути запуска —
  Free, FastSpring, внутреннем и mock-чекауте: несуществующая точка — 400
  `EGRESS_LOCATION_UNKNOWN`, неживая — 503 `EGRESS_LOCATION_UNAVAILABLE`, до
  создания скана, сессии оплаты или траты бесплатной проверки. Это закреплено
  типом, а не дисциплиной вызывающих: функции, создающие скан или сессию
  оплаты, принимают только `LaunchEgress`, а его делает один
  `resolveLaunchEgressLocation`, и записанная точка берётся из него, а не из
  запроса. Подменить точку
  на живую нельзя ни при запуске, ни при обходе: это тот самый молчаливый
  переход, который D-220 запрещает, только теперь он ещё и печатал бы в отчёте
  чужую страну. Free точку не выбирает и идёт из точки по умолчанию. Проверка
  живости D-225 стала по-точечной (`crawl-egress-monitor.ts`: при старте,
  каждые 5 минут, по запросу, если ответ старше 10 минут): упавшая точка не
  предлагается в интерфейсе, а начатый с ней скан падает как platform-сбой.
  Проба доступности сайта D-223 идёт через выбранную точку и хранит её
  (`SiteReachabilityProbe.egressLocation`): «да» из Киева не доказывает ничего
  про Франкфурт, и чекаут требует совпадения. Трафик считается по точкам в
  новой таблице `CrawlEgressLocationUsage` с ключом (location, month) — сменить
  первичный ключ `CrawlEgressUsage` было бы `DROP CONSTRAINT`, который предыдущий
  релиз не переживает при откате; старые строки перенесены как `ua`, потому
  что другого прокси не было. У старых сканов поля нет, и это «не
  зафіксовано», а не Украина: до D-220 обход шёл с сервера в Германии. Если два
  соседних скана шли из разных точек, блок сравнения называет обе и не зовёт
  разницу «виправлено/нове»; если хоть у одного точка не записана — говорит,
  что часть разницы может быть сетевой. Список доступных точек отдаёт
  `GET /scans/launch-config`, а не бандл: какие страны отвечают — факт
  последних минут. Рядом с полем обязательно сказано, что Performance меряет
  PageSpeed Insights из сети Google, и выбор страны на него не влияет — иначе
  цифры скорости читались бы как «для киевского посетителя».

- **D-229. Paddle убран из кода; в экономике — комиссия FastSpring.** Paddle
  не используется: деньги принимает только FastSpring, а от Paddle оставался
  MockPaddle — `/webhooks/paddle`, mock-ветка `/billing/dev-checkout`, подпись и
  схема события, `PADDLE_WEBHOOK_SECRET`, `FLUXRADAR_ENABLE_MOCK_CHECKOUT`. Это
  был второй, не провайдерский способ выдать платный доступ, пусть и выключенный
  в проде. Удалён весь: покупку создаёт только подписанный FastSpring
  `order.completed`, а бесплатный платный скан делает только внутренний
  allowlist `FLUXRADAR_INTERNAL_FREE_EMAILS` — локально тоже. Тесты, которым
  нужен оплаченный скан, покупают его тем же путём, что и прод:
  `test-utils/purchase-scan.ts` открывает сессию самим `createCheckoutSession`
  (подменены только API сессий FastSpring, проба доступности и egress) и
  прогоняет подписанный `order.completed` через настоящий обработчик. Проверку
  `PADDLE_WEBHOOK_SECRET` сняли и с отката: релиз, на который можно откатиться,
  с 2026-09-06 при отсутствии значения подставляет случайное, так что
  переменную можно удалить из `PRODUCTION_ENV_FILE` — строкой целиком, не
  оставляя пустой: пустое значение старый релиз случайным не подменяет и не
  запускается (`DEPLOYMENT.md`, «Retired variables»). Сами колонки `paddle*`,
  триггеры зеркалирования и их индексы в БД остаются до contract-миграции:
  проба отката запускает Prisma-клиент предыдущего образа, а он эти колонки
  выбирает, и деплой хранит два кандидата на откат — миграция идёт, когда оба
  не старше этого релиза. Чтобы следующий релиз смог их удалить, этот уже
  не объявляет их в `schema.prisma` (и `@default("paddle")` у `provider` —
  все записи задают провайдера явно); в коде до той миграции остаются только
  имена индексов в `prisma-errors.ts` и тесты триггеров в `BILLING-007`. В `econ-validate` комиссия Paddle (5% + $0.50) заменена
  комиссией FastSpring 5.9% + $0.95. FastSpring ставку не публикует — она
  договорная, это самая часто цитируемая базовая, и при другой цифре в
  договоре меняются только две константы. Потолки p95-себестоимости больше не
  зашиты числами: они выводятся из цены, цели маржи 50% и комиссии ($23.30 и
  $51.97 вместо $24.25 и $53.50). Это ужесточение гейта: прогноз с p95 выше
  $23.30 и не выше $24.25 (Basic) или выше $51.97 и не выше $53.50 (Complete)
  раньше проходил ECON-001, а теперь падает с `cost-ceiling`.
  Paddle теперь называют только два вида документов. Исторические остаются как
  есть — это история, а не описание текущего кода: исходный план
  `FluxRadar-Feature-Plan.md`, `PLAN_REVIEW.md`, `PLAN_SCAN_RESUME.md`,
  `TASK_BOARD.md`, `FINAL_REPORT.md`, `CUSTOMER_READINESS_REVIEW.md`, разборы
  `REVIEW_T-*.md` и `TEST_T-06.md`, записи этого файла до D-229. Живые называют
  его только там, где речь о том, что от него осталось вне кода: `DEPLOYMENT.md`
...
- **D-230. Порядок contract-миграции проверяет деплой, а не память.**
  Contract-миграция (маркер `fluxradar:contract-phase`) удаляет то, что старый
  релиз ещё читает, и может идти, только когда ни один релиз, на который
  возможен откат, этого уже не читает. Проба отката это не ловит: она
  запускается после `migrate deploy` и проверяет только живой релиз, а сервер
  хранит ещё один. Поэтому миграция называет предпосылку строкой
  `-- fluxradar:contract-requires <миграция>` — первую миграцию релиза, который
  перестал читать старую форму; релиз «несёт» её, если в его
  `apps/api/prisma/migrations` есть этот каталог, а миграции не удаляются.
  `deploy/contract-phase-gate.sh before-migrate` запускается из
  `pre-migration-snapshot.sh` до снапшота и до миграции и отказывает, если
  предпосылки нет хотя бы у одного кандидата на откат: у живого релиза, у цели
  из `runtime/rollback.env` и у двух самых новых других каталогов релизов —
  их оставляет чистка. Отказ не зависит от `ALLOW_MIGRATION_WITHOUT_BACKUP`:
  дело не в бэкапе. Релиз без самого гейта тоже отказывается. Режим
  `rollback-target` — для ручного отката на произвольный релиз
  (`DEPLOYMENT.md`, «Rolling back by hand»): если contract-деплой упал после
  миграции, чистка не прошла, и релиз старше предпосылки остаётся на диске.
  `rollback-release.sh` его не получил: он откатывает только на цель из
  `rollback.env`, то есть на релиз, живой до деплоя, а его гейт уже проверил.
  `BILLING-007` требует строку предпосылки у каждой contract-миграции и
  проверяет, что названная миграция существует и старше; `DEPLOY-018` гоняет
  гейт по реальной последовательности, `DEPLOY-015` — его место перед
  снапшотом. Отсутствие каталога миграций значит «мигрировать нечего», а
  нечитаемый каталог — сомнение, и гейт отказывает до снапшота; раньше такой
  деплой снимал снапшот и шёл дальше.

- **D-231. Contract-фаза: колонки ушедшего провайдера удалены.** Миграция
  `20260923100000_drop_retired_payment_columns` (маркер
  `fluxradar:contract-phase`) убирает триггеры и функции зеркалирования, колонки
  и индексы `paddle*` и default ушедшего провайдера у трёх колонок `provider`;
  `prisma-errors.ts` больше не узнаёт их индексы, а тесты триггеров в
  `BILLING-007` удалены вместе с триггерами. Данные не теряются: каждое значение
  было копией колонки, которая остаётся. После неё Paddle не остаётся ни в коде,
  ни в схеме — только в истории миграций, которую не переписывают.

  **Когда выкатывать.** После успешного деплоя на сервере остаются активный
  релиз и два кандидата на откат: релиз, который был живым в момент деплоя, и
  релиз перед ним. В момент contract-деплоя оба должны быть D-229 или новее,
  поэтому порядок один: первый релиз с D-229 → минимум один обычный релиз,
  задеплоенный успешно → contract-релиз. D-229 пришёл с #20, но в прод попал
  только релизом `fda0eaa` (#21): деплой самого #20 остановился на снапшоте;
  за ним прошёл `84f91d9`. Сразу за D-229 её выкатывать нельзя:
  вторым кандидатом остался бы релиз до D-229, чей Prisma-клиент выбирает эти
  колонки, и ручной откат на него упал бы на первом же запросе. Проба отката
  этого не поймает — она проверяет только живой релиз. Отдельный случай: если
  contract-деплой упадёт после `migrate deploy`, очистка старых релизов не
  запустится (она идёт только после успешного деплоя), и самый старый каталог —
  релиз до D-229 — останется на диске вместе с образом. Схема к этому моменту
  уже сжата, и ручной откат на него недопустим. Порядок держит гейт D-230:
  миграция называет предпосылкой `20260922100000_egress_locations` —
  единственную миграцию релиза D-229 (#20 вмержен вместе с D-228), — и стадия
  `backup` не пустит её, пока эту миграцию не несёт каждый кандидат на откат,
  то есть пока все они не D-229 или новее. Ручной откат на оставшийся релиз до
  D-229 отклоняет `contract-phase-gate.sh rollback-target`.

  **Обратного пути в SQL нет**, и `down.sql` нет намеренно: повторный прогон
  прежних миграций форму не вернёт — колонки `paddle*` создала init-миграция,
  `ADD COLUMN "provider"` из expand-миграции упадёт на существующих колонках, а
  её backfill идёт в обратную сторону. Путь назад — снапшот, который стадия
  `backup` снимает прямо перед миграцией, и его восстановление теряет всё,
  записанное после. Поэтому эту миграцию нельзя выкатывать с
  `ALLOW_MIGRATION_WITHOUT_BACKUP=true`: без работающего бэкапа стадия `backup`
  остановит деплой, и так и должно быть.

- **D-232 (Action Plan)** — В отчёте Complete владелец по кнопке получает Action
  Plan: короткий упорядоченный список изменений для того, кто чинит сайт, и
  Overview без жаргона сверху, который можно переслать клиенту как есть (это и
  есть executive summary отчёта из §16). Пишет Claude Opus 5 — код-константа
  `ACTION_PLAN_ANTHROPIC_MODEL = 'claude-opus-5'` рядом с
  `DEFAULT_ANTHROPIC_MODEL`, без новой переменной деплоя; генерация по запросу,
  хранение по (скан, язык).

  **Что уходит в Anthropic.** Домен; статус, score и coverage каждого раздела;
  по каждому правилу с открытыми проблемами — id, заголовок из отчёта (есть
  только en/uk, для остальных языков модель переводит), раздел, самая высокая
  открытая severity, число открытых проблем, до трёх адресов страниц без query
  string и все различающиеся тексты рекомендаций правила, от самой тяжёлой
  проблемы, каждый не длиннее 2048 символов. Не уходят
  `evidence_excerpt`, скриншоты, трассы и вообще всё из модуля Analytics: его
  находки — данные Google Search Console и GA4, которым нельзя попадать к
  AI-провайдеру (правило Analytics во входе модуль отвергает до отправки).
  Открытые проблемы Analytics при этом входят в знаменатель Reach. Промпт
  собирается поле за полем из явного входа, проходит `runAiRequest` (redaction,
  caps, контракт ответа) и хранится вместе с планом.

  **Согласие — на кнопке.** Под Generate и Regenerate одна строка: названия
  правил, количества и адреса страниц из этого отчёта отправляются Anthropic;
  нажатие и есть согласие. POST несёт версию notice, которую показала кнопка
  (`action-plan-notice-v1`), устаревшая страница получает 409, версия хранится с
  планом. Предпокупочное уведомление (`core-ai-processing-notice-v3`) и его
  версия не менялись.

  **Когда можно.** Только Complete; статус Completed, Partial, Failed или
  Cancelled **и** выполненный `Job` — Analytics пишется после
  `resolveScanOutcome`, одного статуса мало; хотя бы одна открытая проблема вне
  Analytics. Plan Window — 3 дня после `completedAt` последнего прогона и только
  пока проходит `assertPaidWorkAllowed`: генерация — новая работа, как retry.
  `ENTITLEMENT_DAYS` (30) не трогали, окно — отдельное, более короткое правило;
  отчёт называет концом окна более ранний из двух сроков.
  Готовые планы читаются, пока читается отчёт (`assertPaidReportAccess`).
  Порядок отказов POST фиксирован: свой отчёт, paid work, Complete, готовность,
  окно, есть что планировать, настроенный провайдер (503), 10 запусков в час на
  аккаунт, 100 попыток за сутки на весь продукт (503 «AI temporarily
  unavailable») и атомарный claim. Дневной лимит считается в транзакции claim
  под advisory lock: посчитанный до неё, он пропускал два одновременных запуска
  на 99-й попытке. GET не ограничен. Отчёт опрашивает его цепочкой — следующий
  запрос через 3 секунды после ответа — пока план пишется или скан вот-вот
  станет готов (не дольше двух минут), и перестаёт после первой ошибки.

  **Лимиты и есть защита расходов.** AI cost guard из спецификации в коде не
  существует (`packages/ai/src/quota.ts` лишь считает запросы в памяти), поэтому
  расход держат лимиты: на снимок скана 3 успешные генерации и 6 попыток на все
  языки и один прогон одновременно. Claim — один условный `updateMany` по образцу
  `moduleRetryCount`; прогон старше 5 минут считается мёртвым и перехватывается,
  а время его старта — токен, без которого прогон не запишет результат. Запрос
  к провайдеру обрывается через 4 минуты (120 секунд из исходного плана мало для
  adaptive thinking Opus 5 при 16 000 токенов), так что живой прогон до
  перехвата не доживает. Новый
  план в языке заменяет старый, истории нет. У запроса свои caps (60 000 input /
  16 000 output вместо 8000/2000 из `AI_REQUEST_CAPS`): adaptive thinking Opus 5
  делит `max_tokens` с ответом, и общий cap обрезал бы план. Квоту AI скана и
  registry провайдеров, описывающий проверки AI-visibility, запрос не трогает.
  `"fallbacks": "default"` (beta `server-side-fallback-2026-07-01`) переадресует
  отказ модели по его категории, `modelId` — модель, реально ответившая;
  окончательный `refusal` — неудачная попытка с кодом `refused`, а не
  `provider_unavailable`, и отчёт говорит, что Claude отказался.

  **Хранение.** Своя таблица `ActionPlan`, а не `AiResponseRecord`: та входит в
  CSV-контракт экспорта, а план — мнение AI, не canonical record, и ни JSON, ни
  CSV его не несут. В таблице только готовый контент, поэтому идущая или
  упавшая регенерация не прячет и не теряет текущий план. `ActionPlanAttempt` —
  журнал расходов: статус, код ошибки (никогда не текст провайдера) и usage; по
  нему считается дневной лимит, и ни повторный прогон, ни удаление его не
  чистят. Ни одна из таблиц не ссылается на `Scan` с RESTRICT: после отката
  прежний релиз удаляет сканы, не зная этих таблиц, и RESTRICT остановил бы
  retention и удаление аккаунта на любом скане с планом (случай
  `CheckoutSession`, BILLING-007). План уходит со сканом (`ON DELETE CASCADE`).
  Попытка переживает и скан, и аккаунт (`ON DELETE SET NULL` на обоих
  ключах): удаление отчёта не возвращает дневному лимиту уже потраченные
  деньги, а отвязанная строка ни на кого не указывает. Повторный прогон скана
  (module или platform retry) удаляет его планы и обнуляет оба счётчика:
  проблем, из которых планы написаны, больше нет. Это одна транзакция, и токен
  снимается первым: прогон, закончившийся между двумя шагами, иначе сохранил
  бы план из удалённых проблем. Всё, что отчёт читает о плане, — планы,
  последняя ошибка, бюджет, прогон в работе — берётся только из текущего
  снимка: записанное раньше, чем закончился последний прогон, относится к
  заменённым проблемам. Поэтому прогон, который после отката сделал прежний
  релиз, не сбрасывающий планов, не оставит в отчёте старый план, а
  следующий claim сам обнулит оставшиеся счётчики и удалит старые планы.

  **Содержание.** Overview и не больше 7 Actions в порядке impact over effort.
  Critical-правил в ruleset нет, поэтому гарантии «Critical first» тоже нет;
  когда Low-правило идёт раньше High, модель объясняет это в `why`. Каждая
  Action называет `ruleIds` из входа, правило принадлежит не более чем одной
  Action (разные причины одного правила — шаги внутри неё). Неизвестные
  `ruleIds` отбрасываются, правило остаётся только в первой назвавшей его
  Action, Action без правил выпадает, план без Actions — неудачная попытка.
  Количества проблем модель не пишет.

  **Текст — снимок, счётчики — живые.** Open/total по Action, settled и Reach
  (доля открытых проблем скана, которую покрывает хотя бы одна Action, вместе с
  числом правил) считаются при чтении. Внутри одного скана они двигаются, только
  когда владелец ставит Ignored или False Positive (Resolved ставит следующий
  скан), поэтому интерфейс называет Action «settled» и никогда — «fixed».
  Строки-предупреждения над планом пишет сервер, а не AI, по одной на раздел
  Partial или Unavailable; AI SEO / GEO и Analytics не упоминаются, их находки в
  план не попадают вовсе.

  **Где видно.** Готовый план на выбранном языке встаёт на место FixFirst; если
  план есть только на другом языке, FixFirst остаётся, а строка предлагает
  открыть тот план. Печатный отчёт показывает план после scan summary с меткой
  AI. Basic показывает запертый блок без AI-вызова, который покупает Complete
  через `onUpgrade` (Basic нельзя апгрейдить на месте); Free — один пункт в
  FreeUpsell; страницу тарифов не трогали.
- **D-233 (GEO: два провайдера и web search)** — Вопросы видимости AI SEO / GEO
  задаются **двум** провайдерам вместо одного, и оба отвечают с включённым
  **провайдерским web search**. Реестр — константа
  `GEO_VISIBILITY_PROVIDERS = ['anthropic', 'openai']` в
  `orchestrator/geo.ts`; порядок выполнения — порядок константы, в отчёте
  первым показывается OpenAI (о нём спрашивают клиенты). Причина: без поиска
  каждый ответ был воспоминанием из обучающих данных, `citations` всегда
  оставался пустым, а правило GEO-VIS-004 («официальный домен процитирован»)
  практически не могло сработать; вопрос «что ассистент говорит о сайте
  сегодня» может закрыть только ищущий ассистент. Реализация —
  `docs/PLAN_GEO_PROVIDERS.md`, словарь — `docs/CONTEXT.md`. Ниже записано
  неочевидное, чтобы его не «починили».
  **Поиск — только на вопросах видимости.** Новый флаг `AiRequest.webSearch`
  ставят только они; генерация discovery-вопросов остаётся одним
  Anthropic-запросом без поиска (её контракт нейтральности — D-176), а
  UX/Conversion — Anthropic без поиска (D-217). Бюджет — `maxSearchUnits` (8):
  `max_uses` у Anthropic, `max_tool_calls` у OpenAI. У Anthropic берётся
  **базовый** инструмент `web_search_20250305`: динамическая фильтрация
  (`web_search_20260209+`) выполняется через code execution и не подпадает под
  ZDR. Параметр, который провайдер отверг, — это `Unavailable`, а не молча
  ослабленный запрос.
  **Usage — правда провайдера.** Input-clamp в Anthropic-адаптере снят: контент
  ...
  OpenAI. `citations` — это URL, которые модель процитировала в тексте
  (`url_citation` у OpenAI, `web_search_result_location` у Anthropic), в порядке
  первого появления, без повторов, не больше 32; более широкий список
  источников OpenAI (`include`) не запрашивается — это не цитата и в контракте
  для неё нет поля.
  **Один вопрос — два запроса.** Каждый провайдер получает 2 awareness- и 2–4
  discovery-вопроса, то есть 9–13 AI-запросов на скан вместе с генерацией.
  `sequence` начинается с 1 для каждого провайдера: провайдер уже входит в
  `ai_request_key` (D-015) и в `normalized_resource` находок, поэтому квота,
  идемпотентность и fingerprints остаются различимыми без изменений в
  `geo-rules.ts`. Промпт-версия — `geo-questions-v5-awareness`/`-discovery`.
  **В OpenAI уходит ровно то же, что в Anthropic** для того же вопроса:
  системная инструкция и текст вопроса. `brandFacts` и `pageTitles` пустые, ни
  профильного текста, ни доказательств со страниц, ни данных аккаунта, оплаты
  или Google/Bing tokens — новых данных в промптах не появилось.
  **Непрописанный провайдер падает закрыто.** Без ключа каждый его запрос —
  `ProviderUnavailable`, модуль Partial с уже существующей причиной, score не
  двигается (GEO-METHOD-005). Провайдер никогда не пропускается молча:
  `UnconfiguredProvider` — это обобщённый `UnconfiguredAnthropicProvider`, а
  `RoutingAiProvider` считает запрос к неподключённому провайдеру багом
  вызывающего кода (`AiModuleError`), а не ветвью Unavailable.
  **Согласие — v4 с переходом.** Уведомление до покупки становится
  `core-ai-processing-notice-v4`, называет Anthropic и OpenAI, говорит о web
  search, и веб отправляет `providers: ['anthropic', 'openai']`. Скан,
  купленный под v3, остаётся в своём 30-дневном entitlement, поэтому воркер
  принимает список `ACCEPTED_AI_PROCESSING_NOTICE_VERSIONS = ['…-v3', '…-v4']`:
  на таком retry Anthropic отвечает, а запросы OpenAI получают
  `ConsentMissing` — клиент получает ровно то, на что соглашался. Web search не
  меняет того, на что он соглашался: вопросы те же. v3 убирается из списка
  2026-10-22, через 30 дней после релиза.
  **Registry v2 — константы кода, а не настройки пользователя.**
  `DEFAULT_OPENAI_MODEL = 'gpt-5.6-luna'`: самая дешёвая актуальная модель
  OpenAI с поддержкой web search ($0.20/$1.20 за MTok). Стоимость GEO-запроса
  задают вызовы поиска ($10 за 1 000), а не токены, поэтому более дорогая
  `gpt-5.6-terra` почти не меняла бы себестоимость. `OPENAI_MODEL` переопределяет
  константу; `RETIRED_OPENAI_MODELS` при наличии ключа роняет production-boot по
  имени модели, как `anthropic-config.ts`. Параллельный прогон провайдеров,
  continuation после `pause_turn`, локализованный поиск и новые GEO-правила —
  вне v1.
