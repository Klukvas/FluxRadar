...
- **D-232 (Action Plan)** — В отчёте Complete владелец по кнопке получает Action
  ...
  **Хранение.** Своя таблица `ActionPlan`, а не `AiResponseRecord`: та входит в
  ...
  нему считается дневной лимит, и повторный прогон его не чистит. Обе таблицы
  ссылаются на `Scan` с `ON DELETE CASCADE`, а не RESTRICT: после отката прежний
  релиз удаляет сканы, не зная этих таблиц, и RESTRICT остановил бы retention и
  удаление аккаунта на любом скане с планом (случай `CheckoutSession`,
  BILLING-007); текущий релиз удаляет обе таблицы сам. Повторный прогон скана
  ...
  бы план из удалённых проблем. Последняя ошибка в отчёте — только из текущего
  снимка.
