...
`;
  ...
  // The backend gate's database is configured by the workflow and accepted (or
  // refused) by the harness, and the two used to disagree: the job set
  // TEST_DATABASE_URL, which the harness ignores by name, so every
  // database-backed file skipped itself and the gate still reported green.
  // The guard is asked here with exactly what the workflow exports.
  describe('the database the backend gate runs against', () => {
    const quality = readFileSync(QUALITY_WORKFLOW_PATH, 'utf8');

    function backendGateEnv(): Readonly<Record<string, string>> {
      const job = quality.slice(quality.indexOf('  test-backend:'));
      const env = job.slice(job.indexOf('    env:'), job.indexOf('    services:'));
      return Object.fromEntries(
        [...env.matchAll(/^ {6}([A-Z_]+): (.+)$/gm)].map(([, name, value]) => [
          name as string,
    ...
    it('is one the harness accepts, so no suite skips itself in CI', async () => {
      const env = backendGateEnv();
      const { resolveTestDatabase } = await import('../test-utils/test-database-url.ts');

      expect(resolveTestDatabase(env)).toEqual({
        state: 'ready',
        url: env.FLUXRADAR_TEST_DATABASE_URL,
        databaseName: env.FLUXRADAR_TEST_DATABASE_ACK,
      });
    });

    it('creates the very database it points the harness at', () => {
      const env = backendGateEnv();
      const name = env.FLUXRADAR_TEST_DATABASE_ACK ?? '';
      expect(name).not.toBe('');
      expect(quality).toContain(`POSTGRES_DB: ${name}`);
      expect(env.FLUXRADAR_TEST_DATABASE_URL).toContain(`/${name}`);
    });
  });
});
