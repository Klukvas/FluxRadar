 ...
 *
 * v2 добавила режим рендера и явные API-проверки. v3 добавила страну обхода:
 * два обхода из разных стран — это два измерения, а не тренд (D-228), и
 * находка, «исчезнувшая» вместе со сменой локации, не починена. Прогон под
 * старой версией сравнению не подлежит, и это правильно: ключи не равны,
 * находка остаётся открытой, и ни одна не закрывается по несравнимому материалу.
 */
const CRAWL_SCOPE_KEY_VERSION = 'scope-v3';
...
export function crawlScopeKey(scope: ScanScopeInput): string {
  const normalized = {
    ...
    // Страна, из которой ушёл обход (D-228). Сайт вправе отдать другой ответ
    // из другой страны — а то и не отдать его вовсе, — поэтому прогон из Киева
    // не закрывает находку прогона из Франкфурта и не переиспользует его
    // страницы при resume.
